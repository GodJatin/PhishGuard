# Security Policy

## Supported Versions

Currently, the `main` branch of PhishGuard receives security updates.

## Reporting a Vulnerability

Security is a core focus for PhishGuard. If you discover a vulnerability, please do **NOT** open a public issue.

Instead, please email the maintainers privately or use GitHub's private vulnerability reporting feature.

We will acknowledge your report within 48 hours, and provide a timeline for triage and resolution.

## Best Practices
- Please ensure you are always running the latest version of PhishGuard.
- Regularly update both Node.js and Python dependencies.
- Never commit your `.env` secrets or Supabase service keys to the repository.
